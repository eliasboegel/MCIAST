...
def fit(data_list, skipheader=0, skipfooter=0):
    ...
    for i in range(len(data_list)):
        ...
        vals = sp.optimize.curve_fit(n, data[:, 0], data[:, 1], p0=initial_guess, maxfev=1000, method='lm', xtol=1e-15,
                                     col_deriv=True)
...
def solve(partial_pressures, isotherm_data):
    ...
    # Solve system
    subsol = sp.optimize.root(__func, uarr[:-1], args=(isotherm_data, func_cache, pre_calc_kpress), method='lm',
                              options={'xtol': 1e-15, 'col_deriv': True}).x
...
def __func(uarr, isotherm_data, calc_cache, pre_calcs):
    ...
    calc_cache = isotherm_data[:, 0] * np.log1p(
        np.abs(pre_calcs / calc_cache))  # Removed abs and seems to work as numbers here seem to always be positive
