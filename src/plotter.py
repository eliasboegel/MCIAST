...
class Plotter_mms:
    def __init__(self, solver):
        ...
        self.solver = solver
        self.frame = 0
        self.exit_pressure_history = np.empty((self.solver.params.nt, solver.params.component_names.shape[0]))
        ...
        self.exit_pressure_history[0] = self.solver.u_0[self.solver.params.n_points - 2,
                                        :] / self.solver.params.p_partial_in
        for i in range(solver.params.component_names.shape[0]):  # Loop over components
    ...
    def plot_mms(self, t):
        ...
        print(self.solver.u_1[self.solver.params.n_points - 2, :] / self.solver.params.p_partial_in)

        # Get data
        loadings = self.solver.u_1[self.solver.params.n_points - 1:]
        self.exit_pressure_history[self.frame] = self.solver.u_1[self.solver.params.n_points - 1,
                                                 :] / self.solver.params.p_partial_in
