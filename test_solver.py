...
class TestSolver(unittest.TestCase):
    ...
    def test_solve_function(self):
        params = SysParams()
        params.init_params(t_end=10000, dt=0.0001, y_in=np.asarray([0.5, 0.5]), n_points=100, p_in=2e5, temp=298,
                           c_len=1, u_in=1, void_frac=0.995, disp=[0.004, 0.004], kl=[4.35, 1.47], rho_p=1000,
                           p_out=1.999e5, time_stepping="BE", dimensionless=True, dispersion_helium=0.004)
        solver = Solver(params)
