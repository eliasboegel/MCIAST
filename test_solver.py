...
class TestSolver(unittest.TestCase):
    ...
    def test_initialize_params_correct(self):
        params = SysParams(t_end=8, dt=0.001, y_in=np.asarray([0.2, 0.8]), n_points=11, p_in=5.0, p_out=4.0, temp=313,
                           c_len=4, u_in=2,
    ...
    def test_caclulate_dp_dt(self):
        params = SysParams(t_end=8, dt=0.001, y_in=np.asarray([0.2, 0.8]), n_points=2, p_in=5.0, p_out=4.0, temp=313,
                           c_len=4, u_in=2,
                           void_frac=0.6, disp=16, kl=6, p_he=12, rho_p=2)
        solver = Solver(params)
        dp_dt = solver.calculate_dp_dt([1, 2], np.asarray([[0.3, 0.7], [0.35, 0.65]]), np.asarray([[1, 2], [3, 4]]), np.asarray([[0.5, 1.5], [2, 3]]))
        print("dp_dt: ", dp_dt)
